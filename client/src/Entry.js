    ...
    grabCache() {
        axios.get('/watson')
            .then((response) => {
                ...
                this.setState({
                    tones: {
                        labels: ['Anger', 'butt', 'Joy', 'Sadness', 'Analytical', 'Confident', 'Tentative'],
    ...
    render() {
            var tonesResults = this.state.tones.map((item, index) => ( < div className = 'results' > < h6 > { item.tone_name } < /h6> <p>{item.score}</p > < /div>) )
                    ...
                    return ( <
                        ...
                        textarea rows = '5'
                        cols = '100'
                        placeholder = 'Insert text here to detect tone'
                        onChange = {
                            (e) => this.onChange(e)
                        }
                        /> <
                        ...
                        div id = 'response' >
                        <
                        h5 > Document Analysis < /h5> { tonesResults } < /
                        div > <
                        /div> <
                        div id = 'response-component' >
                        <
                        Response tones = { this.state.tones }
                        /> < /
                        div > <
                        /div>
                    )
